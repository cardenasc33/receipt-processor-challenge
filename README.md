...
I have included several test files to test the backend's functions.  These test files are labeled as `{file-to-test}_test.go` (Example: `evaluations_test.go` to test the functions in  `evaluations.go` located at `receipt-processor-challenge/internal/tools/evaluations.go`)
